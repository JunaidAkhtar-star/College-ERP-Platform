...
9. **Never add `border` or `box-shadow` to cards/panels** — the design language is flat & borderless.
...
9. **Never use `h-screen`** — always use `h-[100dvh]` for full-viewport-height elements. `100dvh` accounts for the dynamic viewport on mobile browsers (collapsing address bars, etc.) and works correctly across all devices.
...
- **No borders on cards** — use subtle background color differences instead (`bg-white` vs `bg-slate-50`).
- **No box-shadow** — flat design, use elevation through background color.
...
- [ ] No borders / no box-shadows on cards
...
- ❌ Hardcoded / mock data
- ❌ `border` or `shadow` utility classes on cards
