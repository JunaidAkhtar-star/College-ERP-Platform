...
const steps = [
  { label: 'Campaigns', route: 'communication-hub' },
  { label: 'Notices', route: 'notice' },
...
export default function EngagementWorkflowBar() {
  ...
  return (
    <nav aria-label="Communication and engagement workflow" className="rounded-2xl bg-white p-4">
      <div className="flex items-center gap-2 px-2 pb-2 pt-1">
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary-50 text-primary">
          <MessageCircleMore className="h-4 w-4" />
        </span>
        <div>
          <p className="text-xs font-semibold text-slate-800">Plan to participation</p>
          <p className="text-[10px] text-slate-600">
            Target, review, deliver, engage and measure outcomes
          </p>
        </div>
      </div>
        ...
        {visible.map((step, index) => {
          ...
          return (
            <div
              ...
              className={`flex cursor-pointer shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium ${
                active
