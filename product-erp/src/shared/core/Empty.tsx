...
const Empty = ({
  ...
  const hasAction = Boolean(pathName && (link || onClick));
  const actionClassName =
    'w-fit rounded-full bg-primary px-6 py-2.5 text-center text-xs font-semibold leading-4 text-white transition-colors duration-200 hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30';
  ...
  return (
    <div className="flex flex-col gap-5 px-6 py-12">
      ...
      <div>
        <h2 className="pb-1.5 text-center text-base font-semibold leading-relaxed text-slate-800!">
          {title}
        </h2>
        <p className="pb-4 text-center text-sm font-normal leading-relaxed text-slate-600! max-w-md mx-auto">
          {subTitle}
        </p>
        {hasAction && (
          <div className="flex justify-center">
            {link ? (
              <Link href={link} className={actionClassName}>
                {pathName}
              </Link>
            ) : (
