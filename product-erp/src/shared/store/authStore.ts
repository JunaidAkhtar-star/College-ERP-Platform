...
interface IAuthState {
   ...
   */
  hydrated: boolean;
  /** Target role while the header is coordinating a portal change. */
  roleTransitionTarget: TSystemRole | null;
  ...
  /** Atomically apply a rotated token pair and the refreshed active-role policy. */
  setRefreshedSession: (
    accessToken: string,
    refreshToken: string,
    activeRole?: IActiveRole | null,
  ...
  /** Replace cached permissions with the current backend role policy. */
  setRoleContext: (activeRole: IActiveRole) => void;
  ...
  beginRoleTransition: (role: TSystemRole) => void;
   ...
   * Fails closed if no active role document is loaded. This prevents protected
   * actions from briefly appearing during hydration or after malformed login data.
   */
...
export const useAuthStore = create<IAuthState>((set, get) => ({
  ...
  roleTransitionTarget: null,

  beginRoleTransition(role) {
    set({ roleTransitionTarget: role });
  },

  endRoleTransition() {
    set({ roleTransitionTarget: null });
  },
  ...
  setRefreshedSession(accessToken, refreshToken, activeRole) {
    saveToLocalStorage(KEY_ACCESS_TOKEN, accessToken);
    saveToLocalStorage(KEY_REFRESH_TOKEN, refreshToken);
    if (activeRole) setLocalStorageItem(KEY_ROLE_DOC, activeRole);
    set({
  ...
  setRoleContext(activeRole) {
    const current = get().activeRole;
    const nextRole = {
      ...activeRole,
      assignedRoles: activeRole.assignedRoles ?? current?.assignedRoles,
    };
    setLocalStorageItem(KEY_ROLE_DOC, nextRole);
