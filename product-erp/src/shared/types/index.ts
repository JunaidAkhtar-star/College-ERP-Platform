...
export interface IActiveRole {
  ...
  name: string;
  displayName: string;
  /** Permissions granted to the selected active role only. */
  permissions: { module: string; actions: string[] }[];
  /** NavItem ObjectIds granted to the selected active role only. */
  allowedNavItems: string[];
  isSystem: boolean;
  /** System role used for ownership and department record-scope policies. */
  baseRole: TSystemRole;
...
export interface ILoginResponse {
  ...
  /** True until the tenant administrator completes the compulsory institution setup. */
