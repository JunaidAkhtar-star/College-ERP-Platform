/**
 * Applies only the compliance permission entry needed by accreditation operations.
 * Dry-run by default. Writes require explicit tenant scope and an approved plan hash.
 */
...
const MIGRATION = "accreditation-role-matrix-v1";
...
const canonical = (permission?: IPermission) =>
  permission ? { module: permission.module, actions: [...permission.actions].sort() } : undefined;
  ...
  for (const tenant of await selectedTenants()) {
    ...
    for (const roleName of GOVERNED_ROLES) {
      ...
      const permissions = Array.isArray(role.permissions)
        ...
        : [];
      const before = permissions.find(({ module }) => module === Module.COMPLIANCE);
      const after = DEFAULT_PERMISSIONS[roleName].find(
        ({ module }) => module === Module.COMPLIANCE,
      );
...
const hashPlan = (changes: IPlannedChange[]) =>
  createHash("sha256")
    .update(
      JSON.stringify(
...
async function run() {
  ...
  for (const change of changes) {
    ...
    const permissions = Array.isArray(current?.permissions)
      ...
      : [];
    const currentEntry = permissions.find(({ module }) => module === Module.COMPLIANCE);
    ...
    await roles.updateOne(
      { _id: change.roleId },
      {
        $set: {
          permissions: [
            ...permissions.filter(({ module }) => module !== Module.COMPLIANCE),
            change.after,
          ],
          updatedAt: new Date(),
        },
  ...
  console.info(`Applied ${changes.length} accreditation role updates. RUN_ID=${runId}`);
}
